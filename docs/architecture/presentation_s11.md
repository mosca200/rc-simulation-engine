...
The original S11 low-poly placeholder at `models/acro_electric_01/aircraft.glb` was superseded by
the G3C-A production asset foundation generated by `tools/generate_acro_electric_01_glb.ps1`.
See `renderer_g3c_aircraft_asset_foundation.md` for its current part, material, and articulation
contract. The local render-mesh coordinate contract remains:
...
- Artist-final textures and cockpit/interior detailing remain outside the G3C-A asset foundation.
