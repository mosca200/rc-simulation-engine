...
2. Enumerate devices:
   ...
   WGI discovery can be asynchronous. This command, raw monitoring, and calibration wait up to two
   seconds while processing gilrs events before declaring that no device is present. A zero-device
   result after that bounded window is still not evidence that the TX16S is unsupported; the
   decisive check remains the viewer's post-window initialization diagnostic.
