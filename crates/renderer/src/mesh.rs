...
///
/// The merged `aircraft_mesh()` is untouched. These synthetic panels give
/// tests and the procedural path separable surfaces with explicit hinge
/// pivots, without splitting the production GLB. Render-body metres, same
/// frame as `aircraft_mesh()`: +X right, +Y up, -Z forward.
#[derive(Debug, Clone, PartialEq)]
...
pub fn articulated_aircraft_mesh() -> ArticulatedAircraftMesh {
    let mut rigid_vertices = Vec::with_capacity(24 * 5);
    let mut rigid_indices = Vec::with_capacity(36 * 5);
    add_box(
        &mut rigid_vertices,
        &mut rigid_indices,
        [-0.11, -0.10, -0.56],
...
/// Default binding table for the articulated fixture (spanwise-X hinges
/// for ailerons/elevator, vertical-Y hinge for rudder, unit gains).
#[must_use]
