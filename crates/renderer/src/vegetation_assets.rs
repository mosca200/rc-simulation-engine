...
impl VegetationAssetSet {
    ...
    pub fn production() -> Self {
        let assets = vec![
            ...
            build_deciduous_variant(2, "field_willow_c"),
            build_conifer_variant(0, "field_pine_a"),
            build_conifer_variant(1, "field_spruce_b"),
            build_conifer_variant(2, "field_fir_c"),
        ];
        Self { assets }
    }
...
pub const VARIANTS_PER_SPECIES_TARGET: usize = 3;
...
mod tests {
    ...
    fn production_set_has_two_species_and_three_variants_each() {
        ...
        assert_eq!(set.len(), SPECIES_TARGET * VARIANTS_PER_SPECIES_TARGET);
        let mut deciduous = 0;
        let mut conifer = 0;
        ...
        assert_eq!(deciduous, VARIANTS_PER_SPECIES_TARGET);
        assert_eq!(conifer, VARIANTS_PER_SPECIES_TARGET);
    }
    ...
    fn ground_contact_is_clean_for_every_asset() {
        for asset in production_asset_set().assets() {
            ...
            assert!(
                bark_min_y <= -0.02,
                "{}: trunk must sink into the ground (min y {bark_min_y})",
                asset.name
            );
            assert!(
                bark_max_y > 1.0,
