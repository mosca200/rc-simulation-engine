...
const STAGING_DIRECTORY_NAME: &str = ".xfoil-bundle-staging-work";
...
pub(crate) enum NotPromotable {
    ...
    #[error("run {index} index field ({found}) does not match enumerated index ({expected})")]
    RunIndexMismatch {
        index: usize,
        found: usize,
        expected: usize,
    },
    #[error("run {index} polar_file {found:?} does not match expected canonical path {expected:?}")]
    RunPolarFileMismatch {
        index: usize,
        found: String,
        expected: String,
    },
    ...
    #[error(
        "run {index} convergence_status mismatch: execution has {execution:?}, validation has {validation:?}"
    )]
    ConvergenceStatusMismatch {
        index: usize,
        execution: String,
        validation: String,
    },
    #[error(
        "run {index} has unknown convergence_status {found:?}; expected \"converged\" or \"unresolved\""
    )]
    UnknownConvergenceStatus { index: usize, found: String },
...
pub fn run_xfoil_evidence_bundle(
    ...
    let staging_root = options.output_dir.join(STAGING_DIRECTORY_NAME);

    // CRITICAL RULE #5: clean stale owned staging from prior interrupted invocation
    ...
    match result {
        Ok(XfoilEvidenceBundleStatus::Built) => {
            match promote_staging_to_output(&staging_root, &options.output_dir) {
                Ok(()) => Ok(XfoilEvidenceBundleStatus::Built),
                Err(error) => {
                    // CRITICAL RULE #4: fail-clean — remove all final + staging artifacts
                    let _ = clean_owned_output(&options.output_dir, &owned_artifacts);
                    let _ = remove_dir_if_exists(&staging_root);
                    Err(error)
                }
            }
...
fn build_evidence_bundle_inner(
    ...
    for input in &evidence_inputs {
        ...
        // CRITICAL RULE: read bytes once — use for SHA256, parse, and copy
        let polar_text =
            std::str::from_utf8(&input.polar_bytes).map_err(|_| NotPromotable::MalformedPolar {
                index: input.index,
                dataset_id: input.run.dataset_id.clone(),
                source: XfoilPolarImportError::HeaderNotFound,
            })?;
        ...
        // CRITICAL RULE #2: use actual convergence status from artifacts
        let convergence_status = map_convergence_status(input.run.convergence_status.as_str())
            .map_err(|_| NotPromotable::UnknownConvergenceStatus {
                index: input.index,
                found: input.run.convergence_status.clone(),
            })?;
        let convergence_str = input.run.convergence_status.as_str();
        ...
        let polar_sha256 = sha256_hex(&input.polar_bytes);
        polar_records.push(PolarRecord {
            index: input.index,
            dataset_id: input.run.dataset_id.clone(),
            reynolds: input.run.reynolds,
            mach: input.run.mach,
            convergence_status: convergence_str.to_owned(),
    ...
    // Copy raw polars using the same bytes already read (no re-read)
    for record in &polar_records {
        let input = evidence_inputs
            .iter()
            .find(|i| i.index == record.index)
            .expect("evidence_inputs and polar_records are built in lockstep");
        let destination = staging_root
            .join(POLAR_DIRECTORY)
            .join(&record.polar_filename);
        fs::write(&destination, &input.polar_bytes).map_err(|source_err| {
...
/// Map canonical convergence status strings to model::ConvergenceStatus.
/// Only "converged" and "unresolved" are valid. Unknown values return Err.
fn map_convergence_status(status: &str) -> Result<ConvergenceStatus, ()> {
    match status {
        "converged" => Ok(ConvergenceStatus::Converged),
        "unresolved" => Ok(ConvergenceStatus::Unresolved),
        _ => Err(()),
    }
}
...
fn validate_promotability(
    ...
    for (index, run_report) in execution.runs.iter().enumerate() {
        // CRITICAL RULE #3: cross-check run index
        if run_report.index != index {
            return Err(NotPromotable::RunIndexMismatch {
                index,
                found: run_report.index,
                expected: index,
            });
        }

        // CRITICAL RULE #3: cross-check polar_file path
        let expected_polar_file = format!("{POLAR_DIRECTORY}/{}", polar_filename(index));
        if run_report.polar_file != expected_polar_file {
            return Err(NotPromotable::RunPolarFileMismatch {
                index,
                found: run_report.polar_file.clone(),
                expected: expected_polar_file,
            });
        }
        ...
        // CRITICAL RULE #2: require exact convergence status agreement
        if validation_dataset.convergence_status != run_report.convergence_status {
            return Err(NotPromotable::ConvergenceStatusMismatch {
                index,
                execution: run_report.convergence_status.clone(),
                validation: validation_dataset.convergence_status.clone(),
            });
        }
        // Validate the convergence status is a known value
        if map_convergence_status(&run_report.convergence_status).is_err() {
            return Err(NotPromotable::UnknownConvergenceStatus {
                index,
                found: run_report.convergence_status.clone(),
            });
        }
        ...
        // CRITICAL RULE: read raw bytes once for SHA256 + parse + copy
        let polar_bytes =
            fs::read(&resolved_polar_path).map_err(|_| NotPromotable::MissingPolar {
                index,
        ...
        inputs.push(EvidenceInput {
            ...
            run: RunMirror {
                ...
                convergence_status: run_report.convergence_status.clone(),
            },
            validation: ValidationMirror {
...
fn promote_staging_to_output(
    ...
    // Track what has been published for fail-clean rollback
    let mut published: Vec<&Path> = Vec::new();

    fs::rename(&staging_bundle, &final_bundle)
        .map_err(|source| XfoilEvidenceBundleError::WriteArtifact {
            path: final_bundle.clone(),
            source,
        })
        .inspect(|_| published.push(&final_bundle))?;

    if let Err(source) = fs::rename(&staging_polar_datasets, &final_polar_datasets) {
        // Rollback already-published artifacts
        for p in &published {
    ...
    published.push(&final_polar_datasets);

    if let Err(source) = fs::rename(&staging_polars, &final_polars) {
        for p in &published {
            if p.is_dir() {
