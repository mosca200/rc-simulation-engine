...
fn print_usage() {
    ...
    println!(
        "  rcsim-app render [--model PATH] [--altitude-m M] [--airspeed-mps MPS] [--throttle VALUE] [--controller-profile PATH] [--start-on-ground] [--record-replay PATH] [--scenery none|flying-field] [--camera pilot|chase] [--camera-fov DEG] [--pilot-position X,Y,Z] [--chase-distance-m M] [--chase-height-m M] [--debug-overlays] [--terrain-debug final|albedo|normal|roughness|macro|detail] [--vegetation-debug final|lod|culling|bounds] [--exposure-ev EV] [--renderer v1|v2] [--rv2-6-validation-scene near|100m|500m|1000m|frontlit|sidelit|backlit --rv2-6-validation-ap on|off]"
    );
    println!(
